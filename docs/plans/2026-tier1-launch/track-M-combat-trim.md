...
- M-F3 **firmware done (Track N phase 2, 2026-09-11)**: `dsp/eda.h` streams the same SCR rule (`scr` events) and the sweat slope (`sweat` every 10 s from the MAX30205); the host still applies the withholding when it scores a session. Host reference: SCR rate on `eda-forehead`/`gsr` (phasic rises 0.5–3 s above 0.5 % of tonic) withheld when the skin-temperature slope exceeds 0.05 °C/min.
- M-F4 host reference done; **single-source half on the helm (Track N phase 2)**: `ppg-q` window quality and an `hr` line every 10 s withheld below 80 % in-range beats. Two-source HR agreement within 5 bpm or one high-quality source (≥ 80 % in-range beats per 10 s window), RMSSD only in still rests ≥ 60 s (assumed still without an IMU, flagged), HR recovery slope over the first minute of rest.
...
- M-M7 **done (v0.15, 2026-09-11)**: electronics integration with three interchangeable power/compute options (`assemble_vp0.py --core nape|slab|none`): nape core base with the harness junction bay, nape pod (Heltec + LiPo + IMU + GSR + amp, three tact buttons, OLED, USB-C), slab pods on the rear-node sockets, belt pack, pancake coil former in each disk, rear-half inner harness channel with cover strips and node grooves. Balance with the electronics as reference masses is in the mass report. Every on-helm core fails the headgear rule like the nodes do; belt-only is the headgear-compatible option.

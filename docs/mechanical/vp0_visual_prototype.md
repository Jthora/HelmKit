...
## 4. Printed parts (print-one full set: 52 pieces + coupon + drill guide, 853 g solid, ~640 g printed; combat trim: 30 pieces, 277 g solid, ~207 g printed)
...
| 14 | `pad_front.stl`, `carrier_ppg.stl`, `carrier_eda.stl`, `pad_hub_L/R.stl`, `pad_rear_L/R.stl` (both trims) | 1, 1, 2, 1 each, 1 each | front plate standing on its bottom edge (brim); carriers plug down (the 1.5 mm flange overhang is fine); hub and rear plates outer face down, pillars up | no |
...
2. Cradle U (PETG), both rear halves, pin-lock, the seven pad frames (carriers epoxied into the front plate's windows, plates screwed on, foam glued over). Tenons in and nailed, racks through the block, two nails in the hole pair whose spaces line up. Wear the cradle alone: it must sit above the ears (the phantom says 4 mm), stay put when you shake your head, and go on and off cap-style without opening the pin-lock. This is the chin-strap decision and the dial-versus-pin-lock decision.
...
7. Combat trim (Track M phase 0): the same cradle with the disks, nexus, crown, visor and pylons off; plugs in the sockets; sensor bar pinned and screwed to the band's front face with the carrier in. Wear it under the sparring headgear you actually own before wiring anything: that is gate M-G1, and the fit report already says the nodes and the nape lock will fight the headgear.

...
def build_assembly(with_head=True):
    ...
    for side, tag in ((+1, "L"), (-1, "R")):
        ...
        P[f"cover_{tag}"] = style(placed(build_port_parts.cover(), L.pod_port_frame(135.0, side)), f"port_cover135.{tag}", "steel", add)
...
def snag_report(P, R):
    ...
    for k, lab in (("arch_L", "crown arch"), ("rear_L", "rear band"), ("panel", "brow panel")):

"""
canon.py -- single source of truth for the HelmKit vp0.7 part set.
...
DISK_IN_Y = 107.0             # back plate's head-side face: 29.5 mm off the skull, 1 mm outside the yoke
...
STALK_LEN = 9.0               # beyond the yoke's outer face: 1 gap + 3 plate + 5 into the boss
...
NODE_OUT_Y = 97.0                        # temple / rear node outer face
TEMPLE_NODE = (69.0, 32.0)               # centre x, length: x 53..85
TEMPLE_NODE_W = (-15.0, 15.0)
HUB_NODE = (10.0, 26.0)                  # centre x, length: x -3..23, above the ear
HUB_NODE_OUT_Y = 100.0                   # 16 thick: room for the 8.3 socket
...
NODE_LEN = 26.0
TEMPLE_NODE_X, REAR_NODE_X = TEMPLE_NODE[0], REAR_NODE[0]
# sockets (cylinder standard) in the nodes
HUB_SOCKET = (HUB_NODE[0], (NODE_IN_Y + HUB_NODE_OUT_Y) / 2.0)      # (10, 92) from the top face down: crown arch peg
REAR_SOCKET = (REAR_NODE[0], (NODE_IN_Y + NODE_OUT_Y) / 2.0)        # (-45, 90.5) from the top face down: pylon peg
TEMPLE_SOCKET = (66.0, (NODE_IN_Y + NODE_OUT_Y) / 2.0)              # (66, 90.5) from the bottom face up: microphone boom
# yoke (disk hanger) on the hub node's outer face
YOKE_Y = (HUB_NODE_OUT_Y, HUB_NODE_OUT_Y + 6.0)    # plate y 100..106
YOKE_X = (-5.0, 25.0)
YOKE_Z = (22.0, 70.0)
YOKE_BOLTS = ((0.0, 55.0), (20.0, 55.0))           # (x, z) M3 through node + plate, heads under the foam, nuts in pockets
YOKE_NAIL = (10.0, 50.0)
HUB_CROSS_Z = CRADLE_Z + HUB_NODE_W[1] - CYL_PIN_Z   # 63: arch peg cross-bolt, nut pocket in the yoke
# brow hinge: external serrated lugs on the temple node's outer face
HINGE_SERR = dict(r_in=5.0, r_out=8.3, teeth=24, height=0.8, phase_ear=7.9, phase_tongue=0.4)   # odd phases: no vertex lands on a mesh line
HINGE_GAP = 6.9                          # 6 mm rail + one 0.8 serration face + 0.1
HINGE_BOLT = "M3 x 40, wave washer + nyloc"
LUG_X = (73.0, 91.0)
LUG_Z = (47.0, 69.0)
LUG_IN_Y = (NODE_OUT_Y - 0.5, 106.0)     # inner lug, grown into the node
LUG_OUT_Y = (106.0 + HINGE_GAP, 106.0 + HINGE_GAP + 6.0)   # 112.9 .. 118.9
LUG_BASE = ((73.0, 93.0), (42.5, 47.5))  # x range, z range of the bridge under the gap; ends 2.5 short of the slider sleeve
BROW_HINGE = (82.0, 106.0 + HINGE_GAP / 2.0, 59.0)   # axis along Y: (82, 109.45, 59)
...
CROWN_W = 20.0                # v0.7: narrower and thicker; the arch only carries the crown pad now, and crown hits bend it in its own plane
CROWN_T = 8.0
CROWN_PAD_T = 12.0
CROWN_LEG_Y = HUB_SOCKET[1]              # 92: leg centreline over the socket
CROWN_FOOT = (19.6, 14.0, 18.0)          # x (0.2 inside the ribbon width: the block reaches the bed in the on-edge print), y (centred on the hub socket), z; 8.3 socket, 2.85 walls
CROWN_LEG_Z0 = CRADLE_Z + HUB_NODE_W[1] + CROWN_FOOT[2]    # 80: ribbon starts on top of the foot block
...
RAIL_SECTION = (6.0, 20.0)              # y thickness x z height
RAIL_X1 = 146.0                         # rail tip: 5 mm past the panel front at reach 0
RAIL_HOLES_X0 = 108.0                   # first reach hole (reach 0): sleeve centre sits behind the panel
RAIL_HOLE_PITCH = 5.0
RAIL_HOLES_N = 6                        # reach 0 .. 25 mm
RAIL_Y = BROW_HINGE[1]                  # 109.45 rail centreline (between the lugs)
SLIDER_SLEEVE = (25.0, 3.0)             # length along x, wall
SLIDER_ARM_X = (96.0, 135.0)            # the arm spans from just inside the sleeve (behind the panel) to the flange
SLIDER_FLANGE_T = 4.0
SLIDER_FOOT_X0 = -30.0                  # flange extends this far behind the panel (panel-local x) as a foot under the arm
SLIDER_TUNNEL = (87.0, 98.0, 50.0, 68.0)   # y0, y1, z0, z1 of the through-tunnel along x in the arm (3 mm walls, 18 mm bridge in print)
SLIDER_PIN = "M3 x 16 with a printed knob; nut trapped in the arm"
BROW_REACH_DEFAULT = 0

# ---------------------------------------------------------------------------
# Antenna pylons (rear node sockets) and microphone boom (temple socket)
# ---------------------------------------------------------------------------
...
MIC_BLOCK = (16.0, 13.0, 12.0)          # under the temple node
MIC_BORE_D = 6.4                        # gooseneck / stiff cable
MIC_BORE_ANGLE = 35.0                   # forward-down from vertical
MIC_CUP = (13.0, 10.2, 14.0)            # OD, ID (electret 9.7), depth
MIC_REACH = (90.0, 25.0, -70.0)         # mouth corner, reference only
...
def report() -> str:
    L = [
        ...
        f"  cradle: band {CRADLE_H}x{CRADLE_T} at z {CRADLE_Z}; temple node x {TEMPLE_NODE}, hub node x {HUB_NODE} (y {NODE_IN_Y}..{HUB_NODE_OUT_Y}), rear node x {REAR_NODE}",
        f"  ports: {CYL_PEG_D} mm pegs in {CYL_SOCKET_D} sockets {CYL_DEPTH} deep; hub top (arch), rear top (pylon), temple bottom (mic)",
        f"  brow: panel {BROW_CENTER_W} at x {BROW_X_IN:.1f}, rails hinged at {BROW_HINGE}, reach 0..{(RAIL_HOLES_N-1)*RAIL_HOLE_PITCH:.0f} in {RAIL_HOLE_PITCH:.0f} mm steps ({RAIL_HOLES_N} notches), tilt in 15 deg notches",
        f"  crown: pegs into the hub nodes, ribbon {CROWN_W}x{CROWN_T} from z {CROWN_LEG_Z0} to apex {CROWN_APEX_Z:.1f}",
        f"  pylons: {PYLON_LEN} mm blades, hinge z {PYLON_HINGE_Z}, {PYLON_ANGLE} deg back; mic boom from the temple socket",

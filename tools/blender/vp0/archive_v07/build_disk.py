...
def back(side=+1):
    ...
    boss = L.revolve("boss", [(r0, C.DISK_PLATE_T - 0.1), (bd / 2, C.DISK_PLATE_T - 0.1), (bd / 2, bh), (r0, bh)], axis="Z", center=(0.0, 0.0, 0.0), segments=120)
    ...
    groove.matrix_world = rot(1.7) @ groove.matrix_world
    L.apply_transform(groove)
    L.cut(plate, groove)
    for sx in (+1, -1):
        # locked position at 90 deg from the notch: detent bump at 78 deg, stop wall from 96 deg
        for ang, tang, h in ((78.0, 1.4, C.BAYONET_DETENT), (98.0, 3.0, g1 - g0 + 0.2)):
            Mb = rot(0.9 + ang + (0.0 if sx > 0 else 180.0))
            rc = (C.STALK_D / 2 + 0.1 + g_r) / 2
            L.union(plate, L.add_box_local("bump", Mb, (rc, 0.0, g0 + h / 2 - 0.1), (g_r - C.STALK_D / 2 - 0.3, tang, h + 0.2)))
    for k in range(C.DISK_SCREWS):
        a = math.radians(30.0 + 360.0 * k / C.DISK_SCREWS)
...
PARTS = {
    ...
    "disk_back": (lambda: back(+1), L.ROT_Y_TO_Z, ["print flat, bayonet boss up; no supports (3.3 mm groove ceiling bridges)",
                                                    "push onto the yoke stalk with the lugs in the notches, turn a quarter turn until it clicks"]),
}

...
"""
build_crown_arch_half.py -- vp0.7 crown arch half: pegged foot, rope spine.

30 x 6 ribbon (chamfered section) from a foot block on the hub node's top
with an 8.3 socket joined to the node's socket by a 30 mm coupler (8 mm
printed peg or aluminium rod, M3 cross-bolt each end), straight up 20 mm,
...
def make():
    ...
    # foot block (flush with the ribbon's inner face, protrudes outboard) + peg
    fx, fy, fz = C.CROWN_FOOT
    foot = L.add_box("foot", (C.CX, C.HUB_SOCKET[1], C.CROWN_LEG_Z0 - fz / 2 + 0.8), (fx, fy, fz + 1.6))   # centred on the socket; 1.6 into the ribbon so its fillet clears the ribbon's start cap

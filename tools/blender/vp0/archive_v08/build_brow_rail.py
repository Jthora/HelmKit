...
and a 7 x 20 rail leaving it at 13 deg above forward to the panel's slider,
with six reach holes every 5 mm.
...
NOTES = ["print lying on the inner (ring) face; no supports",
         "runs on the nexus spool; index pin (3.2 mm nail + spring) in the cradle lug locks the tilt; reach pin M3 x 20"]
...
def make(side=+1):
    ...
    for k in range(C.RAIL_HOLES_N):
        L.cut(ring, L.add_cyl_local("reach", Mr, (C.RAIL_S0 + k * C.RAIL_HOLE_PITCH, yb, 0.0), C.M3_CLEAR_DIA / 2.0, C.RAIL_T + 4.0, axis="Y", verts=16))

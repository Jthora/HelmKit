...
build_port_parts.py -- vp0.4 port standard parts (--out-dir):
  ...
  port_coupler.stl          spare printed rounded 10 mm double-male post with pin holes 7 mm from each end (add-on pylons)
  ...
  port_plug_blank.stl       plug + 16 x 16 x 8 block, pinned, plus a 1.8 mm shear-pin hole: add-on starting point
...
def blank():
    ...
    # breakaway option: a small cross hole for a wooden / 1.5 mm pin instead of the nail
    L.cut(b, L.add_cyl("shear", (0.0, 0.0, -C.PORT_PIN_DEPTH - 5.0), C.SHEAR_PIN_DIA / 2.0, C.PORT_SQ + 6.0, axis="X", verts=16))
    return b
...
PARTS = {
    "port_coupler": (lambda: coupler_part("port_coupler", C.COUPLER_LEN), L.ROT_NONE, [f"{C.COUPLER_LEN:.1f} mm spare double-male post for add-on pylons; print flat or cut tube"]),

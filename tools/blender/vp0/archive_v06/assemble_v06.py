...
"""
assemble_vp0.py -- build every vp0.6 part in the assembly frame, run the reports,
...
import vp0lib as L
import canon as C
import build_pod_shell, build_crown_arch_half, build_apex_block, build_cradle_front, build_pod_bracket
...
def build_assembly(with_head=True):
    ...
    for side, tag in ((+1, "L"), (-1, "R")):
        ...
        P[f"brkf_{tag}"] = style(build_pod_bracket.front(side), f"bracket_front.{tag}", "steel", pods)
        P[f"brkr_{tag}"] = style(build_pod_bracket.rear(side), f"bracket_rear.{tag}", "steel", pods)
        P[f"plug_{tag}"] = style(build_pod_bracket.hinge_plug(side), f"hinge_plug.{tag}", "steel", pods)
        add = collection("AddOns")
        for ang in (135.0, 250.0, 300.0):
    ...
    for side, tag in ((+1, "L"), (-1, "R")):
        w, h, t = fb["side"]
        R[f"foam_side_{tag}"] = style(L.add_box("foam", (5.0, side * (C.CRADLE_SIDE_Y - C.CRADLE_T / 2 - t / 2), C.CRADLE_Z), (w, t, h)), f"foam_side.{tag}", "foam", ref)
        w, h, t = fb["temple"]
    ...
    R["nut"] = style(L.add_hex_prism_local("nut", Mn, (0.0, 0.0, w_floor + sp["working"] + 1.0 + 2.5), 8.0, 5.0, axis="Z"), "nape_nut_M5", "spring", ref)
    return P, R
...
PRINT_ITEMS = [
    ...
    ("bracket_front_L", lambda: build_pod_bracket.front(+1), lambda: build_pod_bracket.PARTS["bracket_front_L"][1], "steel"),
    ("bracket_front_R", lambda: build_pod_bracket.front(-1), lambda: build_pod_bracket.PARTS["bracket_front_R"][1], "steel"),
    ("bracket_rear_L", lambda: build_pod_bracket.rear(+1), lambda: build_pod_bracket.PARTS["bracket_rear_L"][1], "steel"),
    ("bracket_rear_R", lambda: build_pod_bracket.rear(-1), lambda: build_pod_bracket.PARTS["bracket_rear_R"][1], "steel"),
    ("hinge_plug", lambda: build_pod_bracket.hinge_plug(+1), lambda: build_pod_bracket.PARTS["hinge_plug"][1], "steel"),
    ...
    ("port_coupler", build_port_parts.PARTS["port_coupler"][0], lambda: L.ROT_NONE, "steel"),
...
README = """HelmKit vp0.6 -- hand-editable assembly
=======================================
Units: 1 Blender unit = 1 mm. Frame: origin between the ear canals, +X forward,
+Y wearer's LEFT, +Z up. Pods are centred on BRAIN_CORE (10, 0, 35) and touch
nothing: the CRADLE holds the head.

Collections
  Cradle           cradle_front (forehead + sides + temple/rear nodes, one PETG print), rear_band.L/.R
                   (tenon into the rear nodes, racks), Nape: body / cover / lid / dial / key / pinion
  Pods.L / Pods.R  pod_shell (paraboloid, dome out, six pinned 20 mm ports); bracket_front (13 deg port ->
                   temple node), bracket_rear (175 deg port -> rear node), hinge_plug (top port, crown clevis)
  Crown            crown_arch.L/.R (tongue hinged in the plugs, rope spine, fold flat), apex_block
  Brow             brow_center (LED bay), brow_wing.L/.R (hinged on the temple nodes, detent), brow_lid
  AddOns           covers on the free ports 135 / 250 / 300 (antenna pylons go here)
  Reference        NOT printed: head, foam blocks, crown / nape pads, the nape spring and nut
  PrintLayout      hidden; every printed part in print orientation on a 325x325 bed

Nape dial: PULL. Turn to tighten (clicks). Pinch the two ribs in the window, pull 1.5 mm, turn to
loosen. Spring 3/8 x 3/4 in in the dial's well against a washer + M5 nyloc: the nut sets the preload.
Brow: flips up about the temple-node hinges (M3, serrated detent). Crown arch: folds at the plugs.
Pods off: two M3 per pod (bracket webs) and the arch bolts -> flat pack.
Every socket: nail through both walls and the post, head counterbored and epoxied.
Chin strap: Y from the slot pairs behind the temple nodes and on the rear halves.
...
def collision_report(P, label):
    ...
    expected = {("pod", "brkf"), ("pod", "brkr"), ("pod", "plug"), ("pod", "cover"), ("cradle", "brkf"), ("cradle", "brkr"),
                ("cradle", "rear"), ("cradle", "wing"), ("plug", "arch"), ("arch", "arch"), ("arch", "apex"),
                ("panel", "lid"), ("panel", "wing"), ("rear", "nape"), ("nape", "nape")}
...
def snag_report(P, R):
    ...
    for k, lab in (("cradle", "cradle U"), ("rear_L", "rear band"), ("arch_L", "crown arch"), ("panel", "brow centre"), ("wing_L", "brow wing"), ("pod_L", "pod")):
